...
##then
  ...
  #DOCKER_BUILDKIT=1 docker buildx inspect --bootstrap
    ...
    #output2="$(
    DOCKER_BUILDKIT=1 docker buildx build --output type=image,dest=$HOME/myimage.tar,compression=uncompressed,force-recompress=true --platform $PLATFORM -f $OPENPILOT_DIR/$DOCKER_FILE $OPENPILOT_DIR --progress=plain
    #2>&1)"
