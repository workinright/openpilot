...
##then
  ...
  DOCKER_BUILDKIT=1 docker buildx inspect --bootstrap
    ...
    #output2="$(
    DOCKER_BUILDKIT=1 docker buildx build --builder mybuilder --output type=docker,dest=$HOME/myimage.tar,compression=uncompressed,force-recompress=true --platform $PLATFORM --progress=plain -f $OPENPILOT_DIR/$DOCKER_FILE $OPENPILOT_DIR
    #2>&1)"
